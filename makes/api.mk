...
API_NODE_PACKAGE_MANIFEST       ?= $(CONFIG_DIR)/package.json
API_NODE_PACKAGE_MANIFEST_ABS   := $(abspath $(API_NODE_PACKAGE_MANIFEST))
API_NODE_LOCKFILE               ?= $(CONFIG_DIR)/package-lock.json
...
api-install: | $(VENV)
	...
	@if [ "$(API_ENABLE_NODE_TOOLS)" = "1" ]; then $(MAKE) node_deps; fi
...
$(API_NODE_READY_MARKER):
	...
	@case "$(API_NODE_BOOTSTRAP_MODE)" in \
	  ...
	  npm-install-pinned) \
	    ...
	    echo "→ Bootstrapping Node toolchain in $(API_NODE_DIR_ABS)"; \
	    cd "$(API_NODE_DIR_ABS)" && { test -f package.json || npm init -y >/dev/null; }; \
	    if ! npm view "$(OPENAPI_GENERATOR_NPM_PACKAGE)" version >/dev/null 2>&1; then \
	      echo "✘ Requested npm package version not found: $(OPENAPI_GENERATOR_NPM_PACKAGE)"; \
	      exit 1; \
	    fi; \
	    cd "$(API_NODE_DIR_ABS)" && { \
	      NPM_CONFIG_CACHE="$(API_NODE_DIR_ABS)/.npm-cache" \
	      npm install --no-fund --no-audit --loglevel=info --save-dev --save-exact "$(REDOCLY_NPM_PACKAGE)" "$(OPENAPI_GENERATOR_NPM_PACKAGE)" > npm-install.log 2>&1 \
	        || { echo "✘ npm install failed — see $(API_NODE_DIR_ABS)/npm-install.log"; tail -n 200 npm-install.log; exit 1; }; \
	  ...
	  npm-ci-sandbox) \
	    ...
	    ARCH_RAW="$$(uname -m)"; \
	    case "$$ARCH_RAW" in x86_64) ARCH_TAG="x64" ;; aarch64|arm64) ARCH_TAG="arm64" ;; *) ARCH_TAG="$$ARCH_RAW" ;; esac; \
	    NODE_TARBALL="node-$(NODE_DIST_VERSION)-$$PLATFORM-$$ARCH_TAG"; \
	    NODE_DST="$(API_NODE_DIR_ABS)/node$(NODE_REQUIRED)"; \
	    ...
	    cp "$(API_NODE_PACKAGE_MANIFEST_ABS)" "$(API_NODE_LOCKFILE_ABS)" "$(API_NODE_DIR_ABS)"/; \
	    cd "$(API_NODE_DIR_ABS)" && { \
	      NPM_CONFIG_CACHE="$(API_NODE_DIR_ABS)/.npm-cache" \
	      npm ci --ignore-scripts --no-fund --no-audit --loglevel=info > npm-install.log 2>&1 \
	        || { echo "✘ npm ci failed — see $(API_NODE_DIR_ABS)/npm-install.log"; tail -n 200 npm-install.log; exit 1; }; \
	    } ;; \
	  *) \
	    echo "✘ Unsupported API_NODE_BOOTSTRAP_MODE=$(API_NODE_BOOTSTRAP_MODE)"; exit 1 ;; \
	esac
...
api-repro:
	@if [ "$(API_ENABLE_REPRO)" != "1" ]; then \
	  echo "→ api-repro is not configured for $(PROJECT_SLUG)"; \
	  exit 0; \
	fi
...
##@ API
api:            ## Run the package API workflow
api-install:    ## Install API toolchain requirements
api-lint:       ## Validate the package OpenAPI schemas
api-test:       ## Start the package API and run schemathesis checks
api-serve:      ## Serve the package API in the foreground
api-serve-bg:   ## Serve the package API in the background
api-stop:       ## Stop the package API background process
api-clean:      ## Remove API artifacts
openapi-drift:  ## Run the package OpenAPI drift check when configured
api-repro:      ## Rebuild the package API toolchain from scratch when configured
endif

ifeq ($(API_MODE),live-contract)
