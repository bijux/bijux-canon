...
publish: check-version build twine-check
	@if [ "$(PUBLISH_UPLOAD_ENABLED)" = "1" ]; then \
	  $(MAKE) twine-upload; \
	  echo "✔ Published $(PUBLISH_PACKAGE_NAME) $(PKG_VERSION) to $(TWINE_REPOSITORY)"; \
	else \
...
publish-test: check-version build twine-check
	@if [ "$(PUBLISH_TEST_ENABLED)" = "1" ]; then \
	  ...
	  echo "✔ Published $(PUBLISH_PACKAGE_NAME) $(PKG_VERSION) to testpypi"; \
	else \
...
twine-upload: ensure-dists
	@echo "→ Uploading $(PUBLISH_PACKAGE_NAME) $(PKG_VERSION) to repository '$(TWINE_REPOSITORY)'"
