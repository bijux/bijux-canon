...
SKIP_PYRIGHT 	?= 1
SKIP_PYDOCSTYLE ?= 1
MYPY_CORE_CONFIG ?= $(CONFIG_DIR)/mypy-core.ini
MYPY_EXTENDED_CONFIG ?= $(CONFIG_DIR)/mypy-extended.ini
...
.PHONY: lint lint-artifacts lint-file lint-dir lint-clean fmt fmt-artifacts mypy-core mypy-extended
...
fmt-artifacts: | $(VENV)
	...
	@$(RUFF) format --config $(CONFIG_DIR)/ruff.toml $(LINT_DIRS) 2>&1 | tee "$(FMT_LOG)"
...
lint-artifacts: | $(VENV)
	...
	@set -euo pipefail; $(RUFF) check --fix --config $(CONFIG_DIR)/ruff.toml --cache-dir "$(RUFF_CACHE_DIR)" $(LINT_DIRS) 2>&1 | tee "$(LINT_ARTIFACTS_DIR)/ruff.log"
	@if [ "$(ENABLE_MYPY)" = "1" ]; then \
	  set -euo pipefail; \
	  $(MYPY) --config-file "$(MYPY_CORE_CONFIG)" --strict --cache-dir "$(MYPY_CACHE_DIR)" 2>&1 | tee "$(LINT_ARTIFACTS_DIR)/mypy-core.log"; \
	else \
	  echo "→ Skipping mypy (set ENABLE_MYPY=1 to enable)" | tee "$(LINT_ARTIFACTS_DIR)/mypy-core.log"; \
	fi
	@if [ "$(SKIP_PYRIGHT)" = "1" ]; then \
	  echo "→ Skipping pyright" | tee "$(LINT_ARTIFACTS_DIR)/pyright.log"; \
	else \
	  set -euo pipefail; $(PYRIGHT) --project $(CONFIG_DIR)/pyrightconfig.json 2>&1 | tee "$(LINT_ARTIFACTS_DIR)/pyright.log"; \
	fi
...
mypy-core:
	@set -euo pipefail; $(MYPY) --config-file "$(MYPY_CORE_CONFIG)" --strict --cache-dir "$(MYPY_CACHE_DIR)"

mypy-extended:
	@set -euo pipefail; $(MYPY) --config-file "$(MYPY_EXTENDED_CONFIG)" --cache-dir "$(MYPY_CACHE_DIR)"
