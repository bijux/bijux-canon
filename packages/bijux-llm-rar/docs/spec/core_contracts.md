...
4. All artifacts SHALL live under `artifacts/bijux-llm-rar/runs/<run_id>/`; writes elsewhere are forbidden.
...
- Artifacts must live under `artifacts/bijux-llm-rar/runs/<run_id>/`.
...
- Allowing writes outside artifacts/bijux-llm-rar/ → rejected because provenance and determinism would be unenforceable.
