...
- **Artifact pollution:** No root writes; artifacts confined to `artifacts/bijux-llm-rar/`.
...
### Trust Boundaries
- Filesystem: only `artifacts/bijux-llm-rar/` is writable; evidence paths are validated.
...
- Trust assumptions: artifacts/bijux-llm-rar/ is the only writable root; ProblemSpecs are not sanitized beyond invariants; locale/time do not affect determinism.
