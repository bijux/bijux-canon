# Why bijux-canon-index is not a vector DB (concrete comparison)

bijux-canon-index is an execution engine; vector DBs are storage/serving systems. Key differences:
...
- No CRUD semantics: bijux-canon-index does not expose database operations; ingestion/materialization are execution-scoped, not general storage APIs.

Use bijux-canon-index when you need audited, contract-bound vector execution. Use a vector DB when you need a managed serving tier with CRUD and scale-out storage.
