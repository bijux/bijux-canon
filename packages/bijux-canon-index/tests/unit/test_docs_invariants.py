...
def test_docs_index_points_to_start_here():
    repo_root = pathlib.Path(__file__).resolve().parents[2]
    docs_index = (repo_root / "docs/index.md").read_text()
    assert "user/start_here.md" in docs_index, "docs/index.md must link to start_here"
...
def test_start_here_exists_and_is_linked():
    ...
    assert "user/start_here.md" in index, "docs/index.md must link to start_here"
...
def test_schema_is_referenced_from_api_docs():
    ...
    assert "apis/bijux-canon-index/v1/schema.yaml" in api_docs, (
