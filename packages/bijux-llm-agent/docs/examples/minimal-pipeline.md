...
```bash
python -m bijux_agent.main run examples/tmp/input.txt --out artifacts/bijux-llm-agent/minimal --config examples/default-config.yml
```
...
```bash
ls -R artifacts/bijux-llm-agent/minimal
```
