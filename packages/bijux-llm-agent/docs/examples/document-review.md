...
```bash
python -m bijux_agent.main run path/to/review_corpus --out artifacts/bijux-llm-agent/review_run --config examples/default-config.yml
```

## How to interpret the outputs

- `artifacts/bijux-llm-agent/review_run/result/final_result.json` is the first file to read.
- `artifacts/bijux-llm-agent/review_run/trace/run_trace.json` is what you use to validate:
