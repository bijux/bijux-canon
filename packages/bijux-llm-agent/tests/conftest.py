...
def _assert_within_allowed(path: Path) -> None:
    ...
    if any(segment in resolved.parts for segment in EXEMPT_PATH_SEGMENTS):
        return
    raise RuntimeError(
        "Writes, temporary files, and artifacts must stay under 'artifacts/bijux-llm-agent/test/'"
    )
...
def enforce_git_clean() -> Generator[None, None, None]:
    ...
    new = after - before
    disallowed = [
        path for path in new if not path.startswith("artifacts/bijux-llm-agent/test/")
    ]
    if disallowed:
        pytest.fail(
            "Untracked paths appeared outside artifacts/bijux-llm-agent/test/: "
            + ", ".join(disallowed)
        )
