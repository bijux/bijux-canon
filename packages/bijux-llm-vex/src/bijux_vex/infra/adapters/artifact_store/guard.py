...
"""
Artifact path guard to prevent writes outside artifacts/bijux-llm-vex unless explicitly allowed.
"""
...
ARTIFACT_ROOT = (Path("artifacts") / "bijux-llm-vex").resolve()
...
def assert_artifact_path(path: Path, *, allow_outside: bool = False) -> Path:
    ...
    if ARTIFACT_ROOT not in resolved.parents and resolved != ARTIFACT_ROOT:
        raise ValidationError(
            f"Write blocked outside artifacts/bijux-llm-vex: {resolved}"
        )
    return resolved
