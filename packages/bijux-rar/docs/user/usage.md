...
```bash
python -m bijux_rar verify --trace artifacts/bijux-rar/runs/<run_id>/trace.jsonl --plan artifacts/bijux-rar/runs/<run_id>/plan.json
```

Replay with drift detection:

```bash
python -m bijux_rar replay --trace artifacts/bijux-rar/runs/<run_id>/trace.jsonl
```
