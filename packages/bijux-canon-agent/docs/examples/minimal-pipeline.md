...
```bash
python -m bijux_canon_agent.main run examples/tmp/input.txt --out artifacts/bijux-canon-agent/minimal --config examples/default-config.yml
```
...
```bash
ls -R artifacts/bijux-canon-agent/minimal
```
