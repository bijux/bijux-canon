...
class StatelessStageRunner(FakeStageRunner):
...
class ResourceExhaustionStageRunner(FakeStageRunner):
    """StageRunner that simulates hitting resource limits."""
...
def make_pipeline(
    ...
    return AuditableDocPipeline(
        ...
        stage_runner_agent=cast(StageRunnerAgent, handler),
        ...
        results_dir=str(
            repo_root / "artifacts" / "bijux-canon-agent" / "test" / "pipeline_results"
        ),
    )
...
async def test_resource_exhaustion_sets_termination_reason(
    ...
    handler = ResourceExhaustionStageRunner()
    pipeline = make_pipeline(handler, logger_manager)
    context = {
