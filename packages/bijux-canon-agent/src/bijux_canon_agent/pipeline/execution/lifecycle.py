...
from bijux_canon_agent.config.defaults import PIPELINE_DEFAULTS
...
class PipelineLifecycleMixin:
    ...
    def __init__(
        ...
        results_dir: str = "../bijux-canon-agent_test/results",
    ) -> None:
        ...
        stage_runner_cls = load_stage_runner_agent()
        self.stage_runner: StageRunnerAgent = stage_runner_agent or stage_runner_cls(
            config=config,
            logger_manager=logger_manager,
        )
    ...
    async def get_telemetry(self) -> dict[str, dict[str, Any]]:
        try:
            ...
            agent_metrics = {
                ...
                "StageRunnerAgent": (
                    await self.stage_runner.execution_kernel.get_telemetry()
