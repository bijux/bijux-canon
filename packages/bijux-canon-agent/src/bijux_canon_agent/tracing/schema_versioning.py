...
def _package_version() -> Version:
    try:
        raw_version = metadata.version("bijux-canon-agent")
    ...
    try:
        ...
        raise RuntimeError(f"Invalid bijux-canon-agent version: {raw_version}") from exc
...
def assert_runtime_trace_compatibility(trace_schema_version: int | None = None) -> None:
    ...
    if _package_version() < minimum:
        raise RuntimeError(
            "bijux-canon-agent runtime too old for trace schema "
