...
- Compatibility README and overview guidance now point legacy agent users at
  the numbered canonical handbook route for `bijux-canon-agent`.
...
- Published package metadata now identifies `bijux-agent` as a real
  compatibility alias package for `bijux-canon-agent` instead of a migration
  bridge placeholder.
...
- Updated compatibility package README links to readable markdown hyperlink text and aligned migration/navigation targets with canonical agent handbook routes.
...
- Compatibility handbook links now resolve canonical agent package routes at
  `bijux-canon/bijux-canon-agent/` instead of numbered slug paths.
