# SPDX-License-Identifier: Apache-2.0
# Copyright © 2026 Bijan Mousavi

"""Root package API metadata for ``bijux_canon_ingest``."""

from __future__ import annotations

from bijux_canon_ingest._lazy_exports import LazyExport

LAZY_EXPORTS: dict[str, LazyExport] = {
    "CanonicalIngestError": (
        ".application.canonical_ingest",
        "CanonicalIngestError",
    ),
    "CanonicalIngestRequest": (
        ".application.canonical_ingest",
        "CanonicalIngestRequest",
    ),
    "CanonicalIngestResult": (
        ".application.canonical_ingest",
        "CanonicalIngestResult",
    ),
    "CanonicalIngestRuntime": (
        ".application.canonical_ingest",
        "CanonicalIngestRuntime",
    ),
    "CanonicalCorpusPreparation": (
        ".application.canonical_ingest",
        "CanonicalCorpusPreparation",
    ),
    "AdmissionBudgets": (".domain.source_admission", "AdmissionBudgets"),
    "AdmissionDisposition": (".domain.source_admission", "AdmissionDisposition"),
    "AdmissionEvidence": (".domain.source_admission", "AdmissionEvidence"),
    "AdmissionIssue": (".domain.source_admission", "AdmissionIssue"),
    "AdmissionIssueCode": (".domain.source_admission", "AdmissionIssueCode"),
    "AdmissionResult": (".domain.source_admission", "AdmissionResult"),
    "CanonicalSourceMetadata": (
        ".domain.source_metadata",
        "CanonicalSourceMetadata",
    ),
    "CitationLineageError": (
        ".domain.citation_lineage",
        "CitationLineageError",
    ),
    "CitationLineageErrorCode": (
        ".domain.citation_lineage",
        "CitationLineageErrorCode",
    ),
    "CitationLineageRecord": (
        ".domain.citation_lineage",
        "CitationLineageRecord",
    ),
    "CitationLineageSegment": (
        ".domain.citation_lineage",
        "CitationLineageSegment",
    ),
    "DocumentCitationLineage": (
        ".domain.citation_lineage",
        "DocumentCitationLineage",
    ),
    "ResolvedCitation": (
        ".domain.citation_lineage",
        "ResolvedCitation",
    ),
    "CorpusSnapshot": (".domain.corpus_snapshot", "CorpusSnapshot"),
    "CorpusSnapshotConfiguration": (
        ".domain.corpus_snapshot",
        "CorpusSnapshotConfiguration",
    ),
    "CorpusSnapshotDocument": (
        ".domain.corpus_snapshot",
        "CorpusSnapshotDocument",
    ),
    "CorpusDelta": (".domain.corpus_delta", "CorpusDelta"),
    "CorpusLockError": (".application.corpus_lock", "CorpusLockError"),
    "CorpusLockIssueCode": (
        ".application.corpus_lock",
        "CorpusLockIssueCode",
    ),
    "PublishedCorpusSnapshot": (
        ".domain.corpus_publication",
        "PublishedCorpusSnapshot",
    ),
    "AppConfig": (".config.app", "AppConfig"),
    "DocsReader": (".config.ingest", "DocsReader"),
    "DiscoveredSource": (".domain.source_discovery", "DiscoveredSource"),
    "DiscoveryIssue": (".domain.source_discovery", "DiscoveryIssue"),
    "DiscoveryIssueCode": (".domain.source_discovery", "DiscoveryIssueCode"),
    "DiscoveryLimits": (".domain.source_discovery", "DiscoveryLimits"),
    "DiscoveryPolicy": (".domain.source_discovery", "DiscoveryPolicy"),
    "DiscoveryResult": (".domain.source_discovery", "DiscoveryResult"),
    "DiscoveryRoot": (".domain.source_discovery", "DiscoveryRoot"),
    "DocumentMetadata": (".domain.document_extraction", "DocumentMetadata"),
    "DocumentParseError": (".domain.document_extraction", "DocumentParseError"),
    "DocumentParseIssueCode": (
        ".domain.document_extraction",
        "DocumentParseIssueCode",
    ),
    "DocxBlockRole": (".domain.document_extraction", "DocxBlockRole"),
    "DocxDocumentMetadata": (
        ".domain.document_extraction",
        "DocxDocumentMetadata",
    ),
    "HtmlBlockRole": (".domain.document_extraction", "HtmlBlockRole"),
    "HtmlDocumentMetadata": (
        ".domain.document_extraction",
        "HtmlDocumentMetadata",
    ),
    "HtmlLink": (".domain.document_extraction", "HtmlLink"),
    "MetadataConflict": (".domain.source_metadata", "MetadataConflict"),
    "MetadataField": (".domain.source_metadata", "MetadataField"),
    "METADATA_SOURCE_PRECEDENCE": (
        ".domain.source_metadata",
        "METADATA_SOURCE_PRECEDENCE",
    ),
    "MetadataProvenanceRecord": (
        ".domain.source_metadata",
        "MetadataProvenanceRecord",
    ),
    "MetadataSource": (".domain.source_metadata", "MetadataSource"),
    "MetadataValue": (".domain.source_metadata", "MetadataValue"),
    "MetadataIntegrityCode": (
        ".application.source_metadata",
        "MetadataIntegrityCode",
    ),
    "MetadataIntegrityError": (
        ".application.source_metadata",
        "MetadataIntegrityError",
    ),
    "NewlineStyle": (".domain.document_extraction", "NewlineStyle"),
    "NormalizedSpanMapping": (
        ".domain.source_mapping",
        "NormalizedSpanMapping",
    ),
    "OcrRequiredOutcome": (".domain.document_extraction", "OcrRequiredOutcome"),
    "FSReader": (".interfaces.cli.document_io", "FSReader"),
    "IngestBoundaryDeps": (".config.ingest", "IngestBoundaryDeps"),
    "IngestConfig": (".config.ingest", "IngestConfig"),
    "IngestDeps": (".config.ingest", "IngestDeps"),
    "ParsedBlock": (".domain.document_extraction", "ParsedBlock"),
    "ParsedDocument": (".domain.document_extraction", "ParsedDocument"),
    "ParsedDocxBlock": (".domain.document_extraction", "ParsedDocxBlock"),
    "ParsedDocxDocument": (".domain.document_extraction", "ParsedDocxDocument"),
    "ParsedHtmlBlock": (".domain.document_extraction", "ParsedHtmlBlock"),
    "ParsedHtmlDocument": (".domain.document_extraction", "ParsedHtmlDocument"),
    "ParsedPdfDocument": (".domain.document_extraction", "ParsedPdfDocument"),
    "ParsedTextBlock": (".domain.document_extraction", "ParsedTextBlock"),
    "ParsedTextDocument": (".domain.document_extraction", "ParsedTextDocument"),
    "PdfDocumentMetadata": (
        ".domain.document_extraction",
        "PdfDocumentMetadata",
    ),
    "PdfPage": (".domain.document_extraction", "PdfPage"),
    "PdfPageExtraction": (".domain.document_extraction", "PdfPageExtraction"),
    "RawMetadataValue": (".domain.source_metadata", "RawMetadataValue"),
    "SemanticChunk": (".domain.semantic_chunking", "SemanticChunk"),
    "SemanticChunkingPolicy": (
        ".domain.semantic_chunking",
        "SemanticChunkingPolicy",
    ),
    "SourceByteSpan": (".domain.source_mapping", "SourceByteSpan"),
    "SourceModification": (".domain.corpus_delta", "SourceModification"),
    "SourceRename": (".domain.corpus_delta", "SourceRename"),
    "SourceTombstone": (".domain.corpus_delta", "SourceTombstone"),
    "SnapshotPublicationError": (
        ".infra.corpus_snapshot_store",
        "SnapshotPublicationError",
    ),
    "SnapshotRecovery": (".domain.corpus_publication", "SnapshotRecovery"),
    "SourceLocator": (".domain.document_extraction", "SourceLocator"),
    "TextBlockRole": (".domain.document_extraction", "TextBlockRole"),
    "TextEncoding": (".domain.document_extraction", "TextEncoding"),
    "TextTransformation": (".domain.source_mapping", "TextTransformation"),
    "TransformationOperation": (
        ".domain.source_mapping",
        "TransformationOperation",
    ),
    "UnexpectedFailure": (".interfaces.errors.exception_bridge", "UnexpectedFailure"),
    "SourceFormat": (".domain.source_admission", "SourceFormat"),
    "SourceMetadataRecord": (
        ".application.source_metadata",
        "SourceMetadataRecord",
    ),
    "VerifiedCorpusLock": (".application.corpus_lock", "VerifiedCorpusLock"),
    "VerifiedLockedSource": (
        ".application.corpus_lock",
        "VerifiedLockedSource",
    ),
    "admit_source": (".application.source_admission", "admit_source"),
    "assess_ocr_requirement": (
        ".application.document_extraction",
        "assess_ocr_requirement",
    ),
    "admit_sources": (".application.source_admission", "admit_sources"),
    "build_chunk_span_mapping": (
        ".application.source_mapping",
        "build_chunk_span_mapping",
    ),
    "build_corpus_snapshot": (
        ".application.corpus_snapshot",
        "build_corpus_snapshot",
    ),
    "apply_corpus_delta": (".application.corpus_delta", "apply_corpus_delta"),
    "build_document_span_mappings": (
        ".application.source_mapping",
        "build_document_span_mappings",
    ),
    "build_document_citation_lineage": (
        ".domain.citation_lineage",
        "build_document_citation_lineage",
    ),
    "chunk_document_mappings": (
        ".application.semantic_chunking",
        "chunk_document_mappings",
    ),
    "plan_corpus_delta": (".application.corpus_delta", "plan_corpus_delta"),
    "publish_corpus_snapshot": (
        ".application.corpus_publication",
        "publish_corpus_snapshot",
    ),
    "read_published_corpus_snapshot": (
        ".application.corpus_publication",
        "read_published_corpus_snapshot",
    ),
    "recover_corpus_snapshot_store": (
        ".application.corpus_publication",
        "recover_corpus_snapshot_store",
    ),
    "build_ingest_deps": (".config.ingest", "build_ingest_deps"),
    "discover_sources": (".application.source_discovery", "discover_sources"),
    "iter_chunks_from_cleaned": (".application.pipeline", "iter_chunks_from_cleaned"),
    "ingest_corpus": (".application.canonical_ingest", "ingest_corpus"),
    "prepare_corpus": (".application.canonical_ingest", "prepare_corpus"),
    "assemble_corpus_snapshot_manifest": (
        ".application.canonical_ingest",
        "assemble_corpus_snapshot_manifest",
    ),
    "iter_ingest_pipeline": (".application.pipeline", "iter_ingest_pipeline"),
    "iter_ingest_pipeline_core": (".application.pipeline", "iter_ingest_pipeline_core"),
    "load_verified_corpus_lock": (
        ".application.corpus_lock",
        "load_verified_corpus_lock",
    ),
    "make_chunk_stream_fn": (".config.ingest", "make_chunk_stream_fn"),
    "make_ingest_fn": (".config.ingest", "make_ingest_fn"),
    "metadata_record_from_parsed_document": (
        ".application.parsed_metadata",
        "metadata_record_from_parsed_document",
    ),
    "normalize_source_metadata": (
        ".application.source_metadata",
        "normalize_source_metadata",
    ),
    "parse_ingest_config": (".config.ingest_parsing", "parse_ingest_config"),
    "parse_docx": (".application.document_extraction", "parse_docx"),
    "parse_html": (".application.document_extraction", "parse_html"),
    "parse_jats": (".application.document_extraction", "parse_jats"),
    "parse_markdown": (".application.document_extraction", "parse_markdown"),
    "parse_pdf": (".application.document_extraction", "parse_pdf"),
    "parse_text": (".application.document_extraction", "parse_text"),
    "resolve_chunk_citation": (
        ".domain.citation_lineage",
        "resolve_chunk_citation",
    ),
    "resolve_source_locator": (
        ".domain.citation_lineage",
        "resolve_source_locator",
    ),
    "gen_bounded_chunks": (".processing.streaming", "gen_bounded_chunks"),
    "gen_grouped_chunks": (".processing.streaming", "gen_grouped_chunks"),
    "gen_stream_deduped": (".processing.streaming", "gen_stream_deduped"),
    "gen_stream_embedded": (".processing.streaming", "gen_stream_embedded"),
    "safe_rag_pipeline": (".processing.streaming", "safe_rag_pipeline"),
    "SymlinkPolicy": (".domain.source_discovery", "SymlinkPolicy"),
    "stream_chunks": (".processing.streaming", "stream_chunks"),
    "result_map_try": (".interfaces.errors.exception_bridge", "result_map_try"),
    "run_ingest_pipeline": (".application.pipeline", "run_ingest_pipeline"),
    "run_ingest_pipeline_docs": (".application.pipeline", "run_ingest_pipeline_docs"),
    "run_ingest_pipeline_path": (".application.pipeline", "run_ingest_pipeline_path"),
    "try_result": (".interfaces.errors.exception_bridge", "try_result"),
    "unexpected_fail": (".interfaces.errors.exception_bridge", "unexpected_fail"),
    "v_map_try": (".interfaces.errors.exception_bridge", "v_map_try"),
    "v_try": (".interfaces.errors.exception_bridge", "v_try"),
    "write_chunks_jsonl": (".interfaces.cli.document_io", "write_chunks_jsonl"),
}

PUBLIC_API = [
    "__version__",
    "assert_never",
    "RawDoc",
    "AdmissionBudgets",
    "AdmissionDisposition",
    "AdmissionEvidence",
    "AdmissionIssue",
    "AdmissionIssueCode",
    "AdmissionResult",
    "CanonicalSourceMetadata",
    "CitationLineageError",
    "CitationLineageErrorCode",
    "CitationLineageRecord",
    "CitationLineageSegment",
    "CanonicalIngestError",
    "CanonicalIngestRequest",
    "CanonicalIngestResult",
    "CanonicalIngestRuntime",
    "CanonicalCorpusPreparation",
    "CorpusSnapshot",
    "CorpusSnapshotConfiguration",
    "CorpusSnapshotDocument",
    "CorpusDelta",
    "DocumentCitationLineage",
    "CorpusLockError",
    "CorpusLockIssueCode",
    "PublishedCorpusSnapshot",
    "DocumentMetadata",
    "DocumentParseError",
    "DocumentParseIssueCode",
    "DocxBlockRole",
    "DocxDocumentMetadata",
    "HtmlBlockRole",
    "HtmlDocumentMetadata",
    "HtmlLink",
    "MetadataConflict",
    "MetadataField",
    "METADATA_SOURCE_PRECEDENCE",
    "MetadataIntegrityCode",
    "MetadataIntegrityError",
    "MetadataProvenanceRecord",
    "MetadataSource",
    "MetadataValue",
    "NewlineStyle",
    "NormalizedSpanMapping",
    "OcrRequiredOutcome",
    "ParsedBlock",
    "ParsedDocument",
    "ParsedDocxBlock",
    "ParsedDocxDocument",
    "ParsedHtmlBlock",
    "ParsedHtmlDocument",
    "ParsedPdfDocument",
    "ParsedTextBlock",
    "ParsedTextDocument",
    "PdfDocumentMetadata",
    "PdfPage",
    "PdfPageExtraction",
    "RawMetadataValue",
    "ResolvedCitation",
    "SemanticChunk",
    "SemanticChunkingPolicy",
    "SourceByteSpan",
    "SourceModification",
    "SourceRename",
    "SourceTombstone",
    "SnapshotPublicationError",
    "SnapshotRecovery",
    "SourceLocator",
    "TextBlockRole",
    "TextEncoding",
    "TextTransformation",
    "TransformationOperation",
    "SourceFormat",
    "SourceMetadataRecord",
    "VerifiedCorpusLock",
    "VerifiedLockedSource",
    "admit_source",
    "assess_ocr_requirement",
    "admit_sources",
    "build_chunk_span_mapping",
    "build_corpus_snapshot",
    "apply_corpus_delta",
    "build_document_span_mappings",
    "build_document_citation_lineage",
    "chunk_document_mappings",
    "plan_corpus_delta",
    "publish_corpus_snapshot",
    "read_published_corpus_snapshot",
    "recover_corpus_snapshot_store",
    "metadata_record_from_parsed_document",
    "normalize_source_metadata",
    "parse_docx",
    "parse_html",
    "parse_jats",
    "parse_markdown",
    "parse_pdf",
    "parse_text",
    "resolve_chunk_citation",
    "resolve_source_locator",
    "DiscoveredSource",
    "DiscoveryIssue",
    "DiscoveryIssueCode",
    "DiscoveryLimits",
    "DiscoveryPolicy",
    "DiscoveryResult",
    "DiscoveryRoot",
    "SymlinkPolicy",
    "discover_sources",
    "CleanDoc",
    "ChunkWithoutEmbedding",
    "Chunk",
    "RagEnv",
    "TextNode",
    "TreeDoc",
    "clean_doc",
    "chunk_doc",
    "iter_chunk_spans",
    "iter_chunk_doc",
    "ingest_corpus",
    "prepare_corpus",
    "assemble_corpus_snapshot_manifest",
    "iter_overlapping_chunks_text",
    "embed_chunk",
    "structural_dedup_chunks",
    "DedupIterator",
    "structural_dedup_lazy",
    "identity",
    "compose",
    "flow",
    "producer_pipeline",
    "pipe",
    "fmap",
    "ffilter",
    "flatmap",
    "tee",
    "probe",
    "StageInstrumentation",
    "instrument_stage",
    "FakeTime",
    "Ok",
    "Err",
    "Result",
    "ErrInfo",
    "make_errinfo",
    "is_ok",
    "is_err",
    "map_result",
    "map_err",
    "bind_result",
    "recover",
    "unwrap_or",
    "to_option",
    "Option",
    "Some",
    "NoneVal",
    "NONE",
    "is_some",
    "is_none",
    "map_option",
    "bind_option",
    "unwrap_or_else",
    "option_from_nullable",
    "option_to_nullable",
    "map_result_iter",
    "filter_ok",
    "filter_err",
    "partition_results",
    "result_map",
    "result_and_then",
    "assert_acyclic",
    "flatten",
    "recursive_flatten",
    "iter_flatten",
    "iter_flatten_buffered",
    "flatten_via_fold",
    "max_depth",
    "fold_tree",
    "fold_tree_no_path",
    "fold_tree_buffered",
    "scan_tree",
    "linear_reduce",
    "linear_accumulate",
    "fold_count_length_maxdepth",
    "scan_count_length_maxdepth",
    "lru_cache_custom",
    "memoize_keyed",
    "DiskCache",
    "content_hash_key",
    "try_map_iter",
    "par_try_map_iter",
    "tap_ok",
    "tap_err",
    "recover_iter",
    "recover_result_iter",
    "split_results_to_sinks",
    "split_results_to_sinks_guarded",
    "ResultsBoth",
    "fold_results_fail_fast",
    "fold_results_collect_errs",
    "fold_results_collect_errs_capped",
    "fold_until_error_rate",
    "all_ok_fail_fast",
    "collect_both",
    "BreakInfo",
    "short_circuit_on_err_emit",
    "short_circuit_on_err_truncate",
    "circuit_breaker_rate_emit",
    "circuit_breaker_rate_truncate",
    "circuit_breaker_count_emit",
    "circuit_breaker_count_truncate",
    "circuit_breaker_pred_emit",
    "circuit_breaker_pred_truncate",
    "with_resource_stream",
    "managed_stream",
    "nested_managed",
    "auto_close",
    "RetryCtx",
    "RetryDecision",
    "retry_map_iter",
    "fixed_policy",
    "exp_policy",
    "is_retriable_errinfo",
    "restore_input_order",
    "ErrGroup",
    "ErrReport",
    "fold_error_counts",
    "fold_error_report",
    "report_to_jsonable",
    "DocRule",
    "Pred",
    "Eq",
    "LenGt",
    "StartsWith",
    "All",
    "AnyOf",
    "Not",
    "RulesConfig",
    "DEFAULT_RULES",
    "eval_pred",
    "any_doc",
    "none_doc",
    "category_startswith",
    "title_contains",
    "abstract_min_len",
    "rule_and",
    "rule_or",
    "rule_not",
    "rule_all",
    "parse_rule",
    "SafeVisitor",
    "assert_rule_is_safe_expr",
    "CleanConfig",
    "DEFAULT_CLEAN_CONFIG",
    "make_cleaner",
    "IngestTaps",
    "DebugConfig",
    "Observations",
    "TraceLens",
    "IngestTrace",
    "IngestConfig",
    "IngestDeps",
    "DocsReader",
    "IngestBoundaryDeps",
    "build_ingest_deps",
    "make_ingest_fn",
    "make_chunk_stream_fn",
    "parse_ingest_config",
    "_trace_iter",
    "gen_chunk_doc",
    "gen_chunk_spans",
    "gen_overlapping_chunks",
    "iter_ingest_pipeline",
    "iter_ingest_pipeline_core",
    "load_verified_corpus_lock",
    "stream_chunks",
    "gen_stream_embedded",
    "gen_stream_deduped",
    "sliding_windows",
    "gen_grouped_chunks",
    "gen_bounded_chunks",
    "safe_rag_pipeline",
    "multicast",
    "throttle",
    "iter_chunks_from_cleaned",
    "run_ingest_pipeline",
    "run_ingest_pipeline_docs",
    "run_ingest_pipeline_path",
    "FSReader",
    "write_chunks_jsonl",
    "AppConfig",
    "Reader",
    "State",
    "Writer",
    "ask",
    "asks",
    "local",
    "get",
    "put",
    "modify",
    "run_state",
    "tell",
    "tell_many",
    "listen",
    "censor",
    "run_writer",
    "wr_pure",
    "wr_map",
    "wr_and_then",
    "transpose_result_option",
    "transpose_option_result",
    "toggle_validation",
    "toggle_logging",
    "toggle_metrics",
    "try_result",
    "result_map_try",
    "v_try",
    "v_map_try",
    "UnexpectedFailure",
    "unexpected_fail",
    "Source",
    "Transform",
    "trace_iter",
    "fence_k",
    "compose2_transforms",
    "compose_transforms",
    "source_to_transform",
    "tap_prefix",
    "make_chain",
    "make_roundrobin",
    "make_merge",
    "fork2_lockstep",
    "make_throttle",
    "make_rate_limit",
    "make_timestamp",
    "make_call_gate",
    "make_tap",
    "make_counter",
    "make_sampler_bernoulli",
    "make_sampler_periodic",
    "make_sampler_stable",
    "make_peek",
    "ensure_contiguous",
    "as_source",
]

__all__ = ["LAZY_EXPORTS", "PUBLIC_API"]
