...
"""Optional `returns` integration helpers.
...
from bijux_canon_ingest.result.types import NONE, Err, ErrInfo, Ok, Option, Result, Some
...
def to_result(obj: Any, *, map_exc: Any | None = None) -> Result[Any, ErrInfo]:
    """Convert a `returns.result.Result` into `bijux_canon_ingest.result.types.Result`.
...
def to_option(obj: Any) -> Option[Any]:
    """Convert a `returns.maybe.Maybe` into `bijux_canon_ingest.result.types.Option`."""
