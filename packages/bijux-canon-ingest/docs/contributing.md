...
| Target | What it does |
| --------------- | --------------------------------------------------------------------------- |
| `make test` | `pytest` + coverage (HTML in `artifacts/bijux-canon-ingest/test/htmlcov/`) |
...
| `make sbom` | CycloneDX SBOM → `artifacts/bijux-canon-ingest/sbom/` |
...
* Coverage report: HTML in `artifacts/bijux-canon-ingest/test/htmlcov/`
...
```bash
make security # bandit + pip-audit
make sbom # CycloneDX, saved to artifacts/bijux-canon-ingest/sbom/
```
