...
Attributes:
    ...
    SRC_DIR (Path): The path to the main source code directory (`src/bijux_agent`).
...
SRC_DIR: Path = Path("src/bijux_agent")
...
def rewrite_links_tree(md: str) -> str:
    ...
    def repl(m: Match[str]) -> str:
        href = m.group(1)
        if href.startswith("src/bijux_agent/") and href.endswith(".py"):
            rel = href[len("src/bijux_agent/") : -3]
            ref = ("reference/" + rel + ".md").replace("\\", "/")
            return f"]({ref})"
        if href.rstrip("/").endswith("src/bijux_agent/commands"):
            return "](reference/commands/index.md)"
        if href in ("#source-code-srcbijux_agent", "#plugin-template-plugin_template"):
            return "](#top)"
        return m.group(0)

    md = _LINK_PAT.sub(repl, md)
    md = md.replace("src/bijux_agent/main.py", "reference/main.md")
    md = md.replace("src/bijux_agent/commands/", "reference/commands/index.md")
    return md
