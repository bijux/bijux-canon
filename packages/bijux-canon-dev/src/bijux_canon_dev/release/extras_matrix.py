...
def _reader_probe(repo_root: Path) -> str:
    ...
    return "\n".join(
        [
            ...
            "        results[name] = {'source_sha256': hashlib.sha256(Path(value).read_bytes()).hexdigest(), 'text_sha256': hashlib.sha256(text.encode()).hexdigest(), 'text_characters': len(text), 'processing_method': result.get('processing_method'), 'ocr_used': result.get('ocr_used')} ",
