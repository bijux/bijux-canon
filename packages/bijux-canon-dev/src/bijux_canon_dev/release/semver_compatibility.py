...
def _root_initializers(repository: Path, revision: str) -> tuple[str, ...]:
    names = _git(repository, "ls-tree", "-r", "--name-only", revision).decode().splitlines()
    return tuple(
        path
        for path in names
...
def _project_scripts(payload: bytes, path: str) -> dict[str, str]:
    ...
    _require(
        all(isinstance(name, str) and isinstance(target, str) for name, target in scripts.items()),
...
def _schema_surface(document: Mapping[str, Any]) -> set[str]:
    ...
    for route, raw_item in paths.items():
        ...
        for method, raw_operation in item.items():
            if method not in _HTTP_METHODS:
                continue
            operation = _object(raw_operation, f"OpenAPI operation is invalid: {method} {route}")
            identity = f"{method.upper()} {route}"
            surface.add(f"operation:{identity}")
            parameters = operation.get("parameters", [])
            _require(isinstance(parameters, list), f"operation parameters are invalid: {identity}")
            for raw_parameter in [*shared, *parameters]:
                parameter = _object(raw_parameter, f"OpenAPI parameter is invalid: {identity}")
                if "$ref" in parameter:
            ...
            responses = _object(operation.get("responses", {}), f"responses are invalid: {identity}")
            surface.update(f"response:{identity}:{code}" for code in responses)
    components = _object(document.get("components", {}), "OpenAPI components are invalid")
    ...
    for name, raw_schema in schemas.items():
        ...
        required = _strings(schema.get("required", []), f"required fields are invalid: {name}")
...
def _deprecation_records(policy: Mapping[str, Any]) -> list[dict[str, str]]:
    ...
    _require(isinstance(raw_records, list) and raw_records, "deprecation policy is missing")
    records: list[dict[str, str]] = []
    for raw_record in cast(list[object], raw_records):
        record = _object(raw_record, "deprecation record is invalid")
        for field in ("alternative", "deprecated_since", "owner", "removal_not_before", "surface"):
            _require(isinstance(record.get(field), str) and record[field], f"deprecation {field} is missing")
        try:
            deprecated = Version(cast(str, record["deprecated_since"]))
            removal = Version(cast(str, record["removal_not_before"]))
        except InvalidVersion as exc:
            raise SemverCompatibilityError("deprecation window has an invalid version") from exc
        _require(deprecated <= candidate < removal, f"deprecation window is closed: {record['surface']}")
...
def compare_release_surfaces(repository: Path, policy_path: Path) -> dict[str, Any]:
    ...
    for path in _root_initializers(repository, prior):
        ...
        _require(current_path.is_file(), f"prior public import root was removed: {path}")
        previous_exports = _public_exports(_revision_file(repository, prior, path), path)
        current_exports = _public_exports(current_path.read_bytes(), path)
        removed = sorted(previous_exports - current_exports)
        _require(not removed, f"prior public imports were removed from {path}: {removed}")
        import_diffs.append(
            {
    ...
    pyprojects = [
        path
        for path in _git(repository, "ls-tree", "-r", "--name-only", prior).decode().splitlines()
        if path.startswith("packages/") and path.endswith("/pyproject.toml") and path.count("/") == 2
    ]
    for path in pyprojects:
        current_path = repository / path
        _require(current_path.is_file(), f"prior distribution metadata was removed: {path}")
        previous_scripts = _project_scripts(_revision_file(repository, prior, path), path)
        current_scripts = _project_scripts(current_path.read_bytes(), path)
        removed = sorted(set(previous_scripts) - set(current_scripts))
        _require(not removed, f"prior installed commands were removed from {path}: {removed}")
    ...
    for path in schema_paths:
        ...
        _require(not removed, f"prior schema surface was removed from {path}: {removed}")
        schema_diffs.append(
            {
    ...
    tests = _strings(policy.get("workspace_acceptance_tests"), "workspace tests are missing")
    _require(tests, "workspace acceptance tests are empty")
    for test in tests:
        _require((repository / test).is_file(), f"workspace acceptance test is missing: {test}")
    rollback = policy.get("workspace_rollback_documentation")
    _require(isinstance(rollback, str) and (repository / rollback).is_file(), "rollback documentation is missing")
...
def _arguments(argv: Sequence[str] | None) -> argparse.Namespace:
    ...
    parser.add_argument("--policy", type=Path, default=Path("configs/semver-compatibility.json"))
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    policy = arguments.policy if arguments.policy.is_absolute() else repository / arguments.policy
    try:
        report = compare_release_surfaces(repository, policy)
    except (OSError, json.JSONDecodeError, SemverCompatibilityError, SyntaxError, tomllib.TOMLDecodeError, yaml.YAMLError) as error:
