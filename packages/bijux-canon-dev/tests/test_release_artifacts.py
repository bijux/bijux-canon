...
def _pyproject_data(package_name: str) -> dict[str, Any]:
...
def test_public_release_packages_ship_source_and_release_docs() -> None:
    ...
    for package_name in sorted(workspace["public_release_packages"]):
        ...
        if not wheel_packages:
            failures.append(
                f"{package_name}: wheel build should declare source packages"
            )
            continue
        ...
        if missing_sdist:
            failures.append(
                f"{package_name}: sdist include missing {', '.join(missing_sdist)}"
            )
...
def test_compatibility_packages_publish_package_local_ignore_rules() -> None:
    ...
    for package_name in sorted(workspace["compat_packages"]):
        ...
        if "exclude" not in sdist or "**/.gitignore" not in sdist["exclude"]:
            failures.append(
                f"{package_name}: sdist exclude should mention **/.gitignore"
            )
    ...
    for package_name in sorted(workspace["public_release_packages"]):
        ...
        for required_line in ("dist/", "build/", "*.egg-info/"):
            if required_line not in text:
                failures.extend(
                    [f"{package_name}: .gitignore should include {required_line}"]
                )
...
def test_generated_version_files_write_to_ignored_build_modules() -> None:
    ...
    for package_name, generated_version_path in GENERATED_VERSION_PACKAGES.items():
        package_root = _package_path(package_name)
        pyproject = _pyproject_data(package_name)
        version_file = pyproject["tool"]["hatch"]["build"]["hooks"]["vcs"][
            "version-file"
        ]
        if version_file != generated_version_path:
            failures.append(
                f"{package_name}: version-file should be {generated_version_path}"
            )

        version_wrapper = package_root / generated_version_path.replace(
        ...
        if generated_version_path not in gitignore_text:
            failures.append(
                f"{package_name}: .gitignore should ignore {generated_version_path}"
            )
