...
def _workspace_distribution_names() -> set[str]:
    workspace = _root_pyproject()["tool"][WORKSPACE_TOOL]
    distribution_names: set[str] = set()

    for package_dir in workspace["package_dirs"].values():
        with (REPO_ROOT / package_dir / "pyproject.toml").open("rb") as handle:
            distribution_names.add(tomllib.load(handle)["project"]["name"])
