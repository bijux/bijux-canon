...
class DuckDBExecutionStore:
    ...
    def _can_bootstrap_latest_schema(self) -> bool:
        ...
        if MIGRATIONS_DIR != DEFAULT_MIGRATIONS_DIR:
            return False
        if SCHEMA_CONTRACT_PATH != DEFAULT_SCHEMA_CONTRACT_PATH:
            return False
        if SCHEMA_HASH_PATH != DEFAULT_SCHEMA_HASH_PATH:
            return False
        rows = self._connection.execute(
            """
            SELECT table_name
        ...
        return table_names.issubset({"schema_migrations"})

    def _bootstrap_latest_schema(self, migrations: dict[int, str]) -> None:
        """Create a fresh database from the canonical schema contract."""
        contract_payload = schema_contracts.load_schema_contract(SCHEMA_CONTRACT_PATH)
        applied_at = datetime.now(tz=UTC).isoformat()
        self._connection.execute("BEGIN")
        try:
            self._connection.execute(contract_payload)
            for version, statement in migrations.items():
                checksum = schema_contracts.hash_payload(statement)
                self._connection.execute(
                    """
