...
flowchart TB
    ...
    proof1["tests/unit for module-level behavior across processing, retrieval, and interfaces"]
    proof1 --> page
    proof2["tests/e2e for package boundary coverage"]
    proof2 --> page
    proof3["tests/invariants for long-lived repository promises"]
    proof3 --> page
    risk1["README.md"]
...
Use `Risk Register` to decide whether `bijux-canon-ingest` has actually earned trust after a change. If one narrow green check hides a wider contract, risk, or validation gap, the work is not done yet.
