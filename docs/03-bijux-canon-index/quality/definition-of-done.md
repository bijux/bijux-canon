...
A change in `bijux-canon-index` is done only when `retrieval and replay behavior` is implemented, explained, and defended together. Local success without package-level trust is not done.
