...
Use `Invariants` to decide whether `bijux-canon-agent` has actually earned trust after a change. If the package passes one narrow check but leaves the wider contract, risk, or validation story unclear, the correct answer is that the work is not done yet.
...
- does `Invariants` show enough proof to trust `bijux-canon-agent` after change
