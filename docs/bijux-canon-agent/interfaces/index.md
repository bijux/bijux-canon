...
Use the interfaces section to see what `bijux-canon-agent` is really asking callers and operators to depend on, and which surfaces are stable enough to treat like contracts.

These pages explain the public face of `bijux-canon-agent`. They exist so a caller can tell which commands, APIs, imports, schemas, and artifacts are deliberate surfaces rather than incidental visibility.
