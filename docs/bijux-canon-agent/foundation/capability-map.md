...
The fastest way to understand `bijux-canon-agent` is to map capabilities to the
...
Use `Capability Map` to decide whether a change makes `bijux-canon-agent` easier or harder to defend as one distinct role in the overall system. If the work makes the package broader without making its role clearer, stop and re-check the boundary before treating the change as a local improvement.
...
- `Capability Map` leaves a reviewer able to explain `bijux-canon-agent` in one clean sentence without hand-waving
...
- prefer keeping `bijux-canon-agent` intelligible as one clear role in the system over making it look universally useful
...
- does `Capability Map` still let a reviewer state `bijux-canon-agent` ownership in one clear sentence
...
A contributor proposes moving new behavior into `bijux-canon-agent` because it looks nearby or related. This page should make it obvious whether that work actually belongs to this package role or whether the overall system becomes clearer if the work lands somewhere else.
...
- `packages/bijux-canon-agent/README.md` plus this section for the shortest maintained explanation of why that boundary exists
