...
- `Ownership Boundary` leaves a reviewer able to explain `bijux-canon-agent` in one clean sentence without hand-waving
...
- `Ownership Boundary` needs repeated exceptions before the package role makes sense
...
- does `Ownership Boundary` still let a reviewer state `bijux-canon-agent` ownership in one clear sentence
