...
- `Change Principles` leaves a reviewer able to explain `bijux-canon-agent` in one clean sentence without hand-waving
...
- `Change Principles` needs repeated exceptions before the package role makes sense
...
- does `Change Principles` still let a reviewer state `bijux-canon-agent` ownership in one clear sentence
