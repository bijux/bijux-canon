...
```mermaid
flowchart LR
    context["bijux-canon-runtime / Quality"]
    ...
    subgraph pressure["Start Here When You Need To Know"]
        direction TB
        q1["what currently proves the bijux-canon-runtime contract instead of merely describing it"]
...
```mermaid
flowchart TB
    promise["Definition of Done<br/>clarifies: see proof | see limitations | judge done-ness"]
    ...
    focus1 --> focus1_2
    focus1_3["tests/regression and tests/smoke for replay and storage protection"]
    focus1 --> focus1_3
    class focus1 ground;
...
Use `Definition of Done` to decide whether `bijux-canon-runtime` has actually earned trust after a change. If one narrow green check hides a wider contract, risk, or validation gap, the work is not done yet.
