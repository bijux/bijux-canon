...
```mermaid
flowchart TB
    promise["Change Validation<br/>clarifies: see proof | see limitations | judge done-ness"]
...
Use `Change Validation` to decide whether `bijux-canon-runtime` has actually earned trust after a change. If one narrow green check hides a wider contract, risk, or validation gap, the work is not done yet.
