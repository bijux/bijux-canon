...
Use the architecture section to understand how `bijux_canon_runtime` is put together before you judge a refactor, a dependency change, or a new seam.

These pages turn `bijux-canon-runtime` from a directory tree into a readable design. They should help a reviewer trace responsibilities, execution paths, and pressure points quickly enough to keep structural conversations grounded.
...
- define what the architecture section covers for bijux-canon-runtime
...
The core architectural claim of `bijux-canon-runtime` is that its structure is deliberate enough for a reviewer to trace responsibilities, dependencies, and drift pressure without reverse-engineering the whole codebase.

## Why It Matters

If the architecture pages for `bijux-canon-runtime` are weak, refactors become guesswork. Dependency drift can hide until it leaks into tests, caller behavior, or operator experience.
...
- `packages/bijux-canon-runtime/src/bijux_canon_runtime` for the actual dependency direction and module structure
- `packages/bijux-canon-runtime/tests` for structural and behavioral regressions that reveal drift
