...
The CLI surface is the operator-facing command layer for `bijux-canon-runtime`. It
...
Use `CLI Surface` to decide whether a caller-facing surface is explicit enough to be depended on. If the surface cannot be tied back to concrete code, schemas, artifacts, and tests, it should be treated as unstable until that evidence exists.
