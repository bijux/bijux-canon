...
| `v2 backup BACKUP_ID` / `v2 restore GENERATION ROOT` | Back up or restore the verified Runtime database and reachable CAS closure. | configured source workspace or absent restore root |
...
requires a destination that does not already exist. These lifecycle commands
protect the Runtime database and its reachable content-addressed artifacts;
they do not claim to copy arbitrary files from the workspace.
