...
Failure recovery for `bijux-canon-runtime` starts by stabilizing evidence around governed run behavior before attempting a fix. Recovery should narrow the problem quickly instead of encouraging random retries.

## What To Check

- start with the artifacts and outputs that expose failure: run records, replay artifacts, and runtime diagnostics
