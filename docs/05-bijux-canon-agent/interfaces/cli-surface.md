...
The CLI surface is the operator-facing command layer for `bijux-canon-agent`. It identifies which commands are deliberate entrypoints and which ones are local implementation detail.
...
flowchart LR
    ...
    command["Command<br/>bijux-canon-agent"]
    modules["Owning boundary<br/>CLI entrypoint in src/bijux_canon_agent/interfaces/cli/entrypoint.py"]
    proof["Where to confirm<br/>tests/integration and tests/e2e for end-to-end"]
...
- move to `https://bijux.io/bijux-canon/05-bijux-canon-agent/operations/` when the caller-facing question becomes procedural or environmental
- move to `https://bijux.io/bijux-canon/05-bijux-canon-agent/quality/` when compatibility or evidence of protection becomes the real issue
- move to `https://bijux.io/bijux-canon/05-bijux-canon-agent/architecture/` when a public-surface question reveals a deeper structural drift
