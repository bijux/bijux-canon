...
flowchart LR
    ...
    entry["Check entry surface<br/>CLI entrypoint in src/bijux_canon_agent/interfaces/cli/entrypoint.py"]
    evidence["Inspect evidence<br/>trace-backed final outputs"]
    rerun["Reproduce with proof<br/>tests/integration and tests/e2e for end-to-end"]
...
This page shows how `bijux-canon-agent` is operated, but it does not replace
