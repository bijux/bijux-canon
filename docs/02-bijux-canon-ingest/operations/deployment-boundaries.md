...
```mermaid
flowchart LR
    package["Package unit<br/>packages/bijux-canon-ingest"]
    metadata["Declared surface<br/>packages/bijux-canon-ingest/pyproject.toml"]
    boundary["Entrypoints and artifacts<br/>CLI entrypoint in src/bijux_canon_ingest/interfaces/cli/entrypoint.py<br/>normalized document trees"]
