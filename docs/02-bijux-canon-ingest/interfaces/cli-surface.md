...
```mermaid
graph TD
    A[CLI Surface] --> B[User command input]
    B --> C[Argument and config validation]
    C --> D[Ingest workflow execution]
    D --> E[Structured output and status]
    E --> F[Operator-facing reliability]
```
