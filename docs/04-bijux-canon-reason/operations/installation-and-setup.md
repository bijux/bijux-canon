...
Installation and setup for `bijux-canon-reason` should tell a maintainer which checked-in files define a valid starting point. If setup for reasoning outputs and verification behavior depends on local folklore, the package is not operationally ready.
