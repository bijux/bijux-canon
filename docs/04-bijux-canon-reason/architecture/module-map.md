...
Treat the architecture pages for `bijux-canon-reason` as a reviewer-facing map
...
```mermaid
flowchart LR
    entry["Boundary<br/>verification<br/>checks and validation outcomes"]
    app["Workflow<br/>reasoning<br/>claim and reasoning semantics"]
    domain["Core responsibility<br/>planning<br/>plan construction and intermediate"]
    infra["Support edge<br/>execution<br/>step execution and tool"]
    entry --> app --> domain
    app --> infra
