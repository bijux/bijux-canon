...
Use the quality section to understand how `bijux-canon-reason` earns trust: which tests matter most, which risks remain visible, and what done should mean after a real change.

These pages explain the proof story for `bijux-canon-reason`. They should make it clear why the package is trustworthy today, what still needs explicit skepticism, and which review questions protect against shallow acceptance disguised as progress.
