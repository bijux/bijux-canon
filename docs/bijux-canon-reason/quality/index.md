...
Use the quality section to understand how `bijux-canon-reason` earns trust: which tests matter, which risks remain visible, and what done should mean after a real change.

These pages explain the proof story for `bijux-canon-reason`. They should make it clear why the package is trustworthy today, what still needs explicit skepticism, and which review questions protect against shallow acceptance.
...
Use `Quality` to decide whether `bijux-canon-reason` has actually earned trust after a change. If the package passes one narrow check but leaves the wider contract, risk, or validation story unclear, the correct answer is that the work is not done yet.
...
- define what the quality section covers for bijux-canon-reason
...
- does `Quality` show enough proof to trust `bijux-canon-reason` after change
