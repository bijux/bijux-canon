...
Use `Lifecycle Overview` to decide whether a change makes `bijux-canon-reason` easier or harder to defend as a bounded package. If the work expands package authority without making ownership clearer, stop and re-check the boundary before treating the change as a local improvement.
