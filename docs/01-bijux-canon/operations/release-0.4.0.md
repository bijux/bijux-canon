...
| `bijux-canon-dev` | Installed wheel, installation, extras, Python, family, semver, secret, placeholder, resource, supply-chain, and candidate verifiers | base for release verification; `[dev]` only for repository development | Internal support distribution; build and validate it with the family but do not add it to the eleven-package public upload matrix |
