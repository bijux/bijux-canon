...
bijux-canon-index treats **user-provided vectors** as the primary, most explicit workflow. Auto-embeddings are available, but only when you opt in with an explicit model.

## Bring Your Own Vectors (Recommended for control)
- Maximum determinism: you control the embedding model, version, and runtime.
- Full auditability: vectors can be regenerated outside bijux-canon-index.
...
```bash
bijux ingest --doc "hello" --vector "[0.1,0.2,0.3]"
```

Auto-embed explicitly:
```bash
bijux ingest --doc "hello" --embed-model "all-MiniLM-L6-v2"
```

Optional caching:
```bash
bijux ingest --doc "hello" --embed-model "all-MiniLM-L6-v2" --cache-embeddings sqlite
```
