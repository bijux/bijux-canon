...
- **Artifact pollution:** No root writes; artifacts confined to `artifacts/bijux-canon-reason/`.
...
### Trust Boundaries
- Filesystem: only `artifacts/bijux-canon-reason/` is writable; evidence paths are validated.
...
- Trust assumptions: artifacts/bijux-canon-reason/ is the only writable root; ProblemSpecs are not sanitized beyond invariants; locale/time do not affect determinism.
