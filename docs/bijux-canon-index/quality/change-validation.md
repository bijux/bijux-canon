...
Use `Change Validation` to decide whether `bijux-canon-index` has actually earned trust after a change. If one narrow green check hides a wider contract, risk, or validation gap, the work is not done yet.
...
- does `Change Validation` show enough proof to trust `bijux-canon-index` after change
