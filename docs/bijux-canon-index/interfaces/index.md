...
Use the interfaces section to see what `bijux-canon-index` is really asking callers and operators to depend on, which surfaces are stable enough to treat like contracts, and which ones are only implementation detail.

These pages explain the public face of `bijux-canon-index`. They exist so a caller can tell which commands, APIs, imports, schemas, and artifacts are deliberate surfaces rather than incidental visibility, and so operators can see what the package is truly prepared to stand behind.
