...
def _generate_api_pages() -> Dict[str, List[Tuple[str, str]]]:
    ...
    for root, _, files in os.walk(SRC_DIR):
        ...
        for file in files:
            ...
            blurb = f"This section documents the internals of the `{module_name}` command in Bijux Agent.\n" if is_command else f"This section documents the internals of the `{module_name}` module in Bijux Agent.\n"
            full_module_path = f"bijux_agent.{module_name}" if section is None else f"bijux_agent.{section.replace(os.sep, '.')}.{module_name}"
            content = (
                PAGE_META_NO_EDIT
                + header
                + blurb
